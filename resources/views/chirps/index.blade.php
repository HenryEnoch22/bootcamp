...
{{--                    @dump($errors->get('message'))--}}
                    <form method="POST" action="{{route('chirps.store')}}">
                        ...
                        <textarea
                            name="message"
                            class="block w-full rounded-md border-gray-300 bg-white shadow-sm transition-colors duration-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 dark:border-gray-600 dark:bg-gray-800 dark:text-white dark: text-white dark:focus:border-indigo-300 dark:focus:ring-indigo-200 dark:focus:ring-opacity-50 mt-2"
                            placeholder="What's on your mind?"
                        >{{old('message')}}</textarea>
                        <x-button class="mt-4">Chirp</x-button>
                    </form>
                </div>
{{--                <x-welcome /> --}}
            </div>

{{--                <div class="mt-6 bg-white dark:bg-gray-800 shadow-sm rounded-lg divide-y dark:divide-gray-900"></div>--}}
            <div class="mt-6 bg-white dark:bg-gray-800 shadow-sm rounded-lg divide-y dark:divide-gray-900">
                @foreach($chirps as $chirp)
                    <div class="p-6 flex space-x-2">
                        ...
                        @can('update',$chirp){{--Verifica que la autenticacion del usuario sea del mismo que el usuario del chirp mediante ChirpPolicy --}}
                            <x-dropdown>
                            <x-slot name="trigger">
                            ...
                            <x-slot name="content">
                                <x-dropdown-link :href="route('chirps.edit', $chirp)">Editar chirp</x-dropdown-link>
                                <form method="POST" action={{route('chirps.destroy', $chirp)}}>
                                    @csrf @method('DELETE')
                                    <x-dropdown-link :href="route('chirps.destroy', $chirp)" onclick="event.preventDefault(); this.closest('form').submit(); ">Eliminar chirp</x-dropdown-link>
                                </form>
